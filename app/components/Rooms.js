...
const Rooms = async ({ data }) => {
    const { title, gallery, text } = data
    return (
        <Link className='relative w-[360px] h-[480px] rounded-2xl overflow-hidden first-of-type:div:hover:hidden services' href={`/habitaciones/${(parseInt(data.id) - parseInt(data.id.slice(1))).toString()}/${data.id}`}>
            <Image src={gallery[0].img} alt={gallery[0].alt} width={0} height={0} className='object-cover w-full h-full' />
